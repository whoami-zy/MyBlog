...
function SimpleTextEditor(id, objectId) {
    ...
    this.getEditorHtml = function() {
        ...
        html += '<td><select onchange="'+this.objectId+'.execCommand(\'formatblock\', this.value);this.selectedIndex=0;"><option value="">正文</option><option value="<h1>">标题一</option><option value="<h2>">标题二</option><option value="<h3>">标题三</option><option value="<p>">段落</option><option value="<pre>">文本格式化</option></select></td>';
        ...
        html += '</tr></table></td></tr>';
        html += '<tr style="display:block;padding-left:50px;"><td class="frame"><iframe id="'+this.id+'-frame" frameborder="0"></iframe></td></tr>';
